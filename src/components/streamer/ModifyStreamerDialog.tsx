...
interface ModifyStreamerDialogProps {
  streamer: Streamer;
}

const ModifyStreamerDialog: React.FC<ModifyStreamerDialogProps> = ({
  streamer,
}) => {
  const { isAuthenticated } = useAuthStore();

  const [isOpen, setIsOpen] = useState(false);
  const [streamerName, setStreamerName] = useState(streamer.name);
  const [description, setDescription] = useState(streamer.description ?? "");
  const [platforms, setPlatforms] = useState(
    streamer.platforms ?? [{ platformName: "", channelUrl: "" }],
  );
  ...
  return (
    <Dialog.Root open={isOpen} role="alertdialog" placement="center">
      <Tooltip
        disabled={isAuthenticated}
        content="스트리머 정보 수정은 로그인 후 이용할 수 있어요"
      ...
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            <Dialog.Header>
              <Dialog.Title>스트리머 정보 수정하기</Dialog.Title>
            </Dialog.Header>
...
export default ModifyStreamerDialog;
